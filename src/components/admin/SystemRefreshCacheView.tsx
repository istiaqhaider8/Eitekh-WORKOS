...
export function SystemRefreshCacheView({ orgId, projectId }: SystemRefreshCacheViewProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2.5 mb-1.5">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-mono">
                v{cacheVersion}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3">
            {userPermissions.canFullRefresh && (
              <button
                ...
                className="px-3 py-1.5 rounded-xl border border-slate-700 bg-slate-800/80 hover:bg-slate-800 text-xs font-semibold text-slate-300 transition-all flex items-center gap-2 cursor-pointer shadow-xs disabled:opacity-50"
              >
      ...
      {lastExecutionResult && (
        <div className="bg-slate-950 border border-emerald-500/40 rounded-2xl p-5 space-y-4 shadow-xl animate-in fade-in duration-300">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">
                  Executed in <span className="text-emerald-400 font-bold">{lastExecutionResult.durationMs}ms</span> • Affected items: {lastExecutionResult.itemsAffected}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {lastExecutionResult.checklist.map((item: any, idx: number) => (
              <div
                key={idx}
                className="p-3 bg-slate-900/80 rounded-xl border border-slate-800 flex items-start gap-2.5 text-xs"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {actionCards.map((card) => {
            ...
            return (
              <div
                key={card.id}
                className={`bg-slate-900/80 border rounded-xl p-5 flex flex-col justify-between transition-all duration-200 relative overflow-hidden ${
                  card.isHero
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="px-2 py-0.5 bg-slate-800/80 text-slate-300 border border-slate-700 rounded text-[10px] font-semibold">
                      {card.badge}
                    </span>
                  </div>
                ...
                <div className="pt-5 mt-3 border-t border-slate-800/80 flex items-center justify-between">
                  <div className="text-[10px] text-slate-500">
                    Scope: <span className="text-slate-300">{card.scope}</span>
                  </div>
