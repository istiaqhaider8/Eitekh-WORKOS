      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/60 p-3 rounded-xl border border-slate-800">
        ...
        <div className="flex items-center gap-2 text-xs">
          <div className="flex items-center gap-1.5 bg-slate-950 px-2.5 py-1.5 rounded-lg border border-slate-800">
            ...
            >
              <option value="all" className="bg-slate-900">All Governance Actions</option>
              <option value="ROLE_CREATED" className="bg-slate-900">ROLE_CREATED</option>
              <option value="ROLE_UPDATED" className="bg-slate-900">ROLE_UPDATED</option>
              <option value="ROLE_CLONED" className="bg-slate-900">ROLE_CLONED</option>
              <option value="ROLE_ACTIVATED" className="bg-slate-900">ROLE_ACTIVATED</option>
              <option value="ROLE_DEACTIVATED" className="bg-slate-900">ROLE_DEACTIVATED</option>
              <option value="ROLE_DELETED" className="bg-slate-900">ROLE_DELETED</option>
              <option value="USER_ADDED_TO_ROLE" className="bg-slate-900">USER_ADDED_TO_ROLE</option>
              <option value="USER_REMOVED_FROM_ROLE" className="bg-slate-900">USER_REMOVED_FROM_ROLE</option>
              <option value="BULK_USERS_ADDED_TO_ROLE" className="bg-slate-900">BULK_USERS_ADDED_TO_ROLE</option>
              <option value="USER_ROLES_UPDATED" className="bg-slate-900">USER_ROLES_UPDATED</option>
            </select>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-950 text-slate-400 uppercase tracking-wider text-[10px]">
                <th className="p-3 font-bold text-slate-300 min-w-[140px]">Timestamp</th>
                <th className="p-3 font-bold text-slate-300 min-w-[160px]">Administrator</th>
                <th className="p-3 font-bold text-slate-300 min-w-[160px]">Action Type</th>
                <th className="p-3 font-bold text-slate-300 min-w-[200px]">Entity / Target</th>
                <th className="p-3 font-bold text-slate-300 min-w-[100px]">Status</th>
                <th className="p-3 font-bold text-slate-300 text-right">Details</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-850/60 transition-colors">
                    ...
                    <td className="p-3">
                      <div className="font-semibold text-slate-200">{log.entityName || log.entityId}</div>
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-xl shadow-2xl p-5 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <span className="text-slate-400 font-bold block mb-1">State Payloads:</span>
                <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 font-mono text-[11px] text-slate-300 max-h-60 overflow-y-auto space-y-2">
