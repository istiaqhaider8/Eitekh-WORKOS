...
export function InspectorTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 bg-slate-950 px-2.5 py-1.5 rounded-lg border border-slate-800 text-xs">
            <span className="text-slate-400 font-medium">Target Project:</span>
            <select
              value={inspectorProjectId}
              onChange={(e) => setInspectorProjectId(e.target.value)}
              className="bg-transparent text-slate-200 font-semibold focus:outline-none cursor-pointer max-w-[200px] truncate"
            >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1">
              ...
              <div className="text-xs text-slate-400">{inspectData.user?.email}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                          ...
                          return (
                            <div
                              ...
                              className={`p-2.5 rounded-lg border text-xs cursor-pointer transition-all space-y-1 ${
                                isSelected
                                  ? 'bg-indigo-600/15 border-indigo-500 text-slate-100 shadow-xs'
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              {selectedTracePerm ? (
                <div className="bg-slate-900 border border-indigo-500/30 rounded-xl p-5 space-y-4 shadow-xs sticky top-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedTracePerm.tracePath?.map((step: any, idx: number) => (
                      <div key={idx} className="flex items-start gap-3 text-xs">
                        ...
                        <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800 text-slate-300 flex-1 leading-relaxed">
                          {typeof step === 'string' ? (
                            step
                            ...
                            <div className="space-y-0.5">
                              ...
                              <div className="text-slate-200">{step.detail || JSON.stringify(step)}</div>
                            </div>
                          ) : (
                            String(step)
                          )}
