  ...
  if (loading && !data) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-sm font-semibold text-slate-400">Compiling Enterprise Platform Reports...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Catalog Sidebar */}
        <div className="lg:col-span-1 bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <span className="text-[11px] font-bold uppercase text-slate-400">15 Report Types</span>
            <select
              value={selectedPeriod}
              onChange={(e) => setSelectedPeriod(e.target.value)}
              className="bg-slate-950 border border-slate-800 rounded px-2 py-0.5 text-[10px] text-slate-300 outline-none"
            >
          ...
          <div className="space-y-1">
            {catalog.map((rep: any) => (
              ...
              >
                ...
                <span className="text-[9px] uppercase px-1.5 py-0.5 rounded bg-slate-950/60 font-mono text-slate-300 shrink-0 ml-1">
                  {rep.category}
                </span>
              </button>
            ))}
        ...
        {/* Report Canvas */}
        <div className="lg:col-span-3 bg-slate-900/90 border border-slate-800 rounded-xl p-5 space-y-6">
          <div className="border-b border-slate-800 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <h3 className="text-base font-bold text-slate-100">{currentReport?.name}</h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Generated at: <span className="font-mono text-slate-300">{new Date(reportData.generatedAt).toLocaleString()}</span> | Period: <span className="font-semibold text-slate-200 uppercase">{selectedPeriod}</span>
              </p>
            </div>

            <div className="text-right text-xs">
              <span className="text-slate-500 block text-[10px]">Audited By</span>
              <span className="font-mono text-slate-300">{reportData.generatedBy}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
              ...
              <div className="text-lg font-bold text-slate-200">{reportData.kpis?.totalOrgs}</div>
            ...
            {selectedReportId.includes('SECURITY') || selectedReportId.includes('THREAT') ? (
              <div className="space-y-2">
                {reportData.activeThreats?.length === 0 ? (
                  <div className="p-6 text-center text-xs text-slate-500 bg-slate-950 rounded-lg">
                    No active threat incidents logged for this period.
                  </div>
                ) : (
                  reportData.activeThreats?.map((t: any) => (
                    <div key={t.id} className="p-3 bg-slate-950 rounded-lg border border-slate-800 flex items-center justify-between text-xs">
                      <div>
                        <div className="font-bold text-slate-200">{t.eventType} - {t.severity}</div>
                ...
                <table className="w-full text-left text-xs text-slate-300">
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {reportData.auditLogs?.slice(0, 10).map((l: any) => (
                      <tr key={l.id}>
                        <td className="p-2.5 text-slate-400 text-[11px]">{new Date(l.createdAt).toLocaleTimeString()}</td>
                        <td className="p-2.5 font-semibold text-slate-200">{l.action}</td>
                        <td className="p-2.5 text-indigo-300">{l.targetResource}</td>
                        <td className="p-2.5 text-slate-400 font-mono text-[11px]">{l.actorId.slice(0, 10)}...</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
