  ...
  if (loading && !data) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-sm font-semibold text-slate-400">Loading Job & Automation Monitor...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase">Recurring Tasks</span>
          <div className="text-lg font-bold text-slate-100">{summary.activeRecurringTasks} / {summary.totalRecurringTasks}</div>
          <p className="text-[10px] text-slate-500">Active schedulers</p>
        </div>
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase">Automation Rules</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase">Webhooks Configured</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase">Email Deliveries</span>
        ...
        {recurringTasks.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {recurringTasks.map((t: any) => (
              <div key={t.id} className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-indigo-400 uppercase">{t.project?.key || 'PROJECT'}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${t.isActive ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-800 text-slate-400'}`}>
                      {t.isActive ? 'ACTIVE' : 'PAUSED'}
                    </span>
                  </div>
                  <h4 className="text-xs font-bold text-slate-100">{t.project?.name}</h4>
                  <div className="text-[11px] text-slate-400 font-mono">Cron: {t.scheduleCron}</div>
                </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {emailLogs.map((e: any) => (
                <tr key={e.id} className="hover:bg-slate-800/40">
                  <td className="p-3 font-medium text-slate-200">{e.to}</td>
                  <td className="p-3 text-slate-300">{e.subject}</td>
                  ...
                  <td className="p-3 text-slate-400">{new Date(e.createdAt).toLocaleTimeString()}</td>
