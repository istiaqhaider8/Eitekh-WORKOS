  ...
  if (loading && !data) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-sm font-semibold text-slate-400">Probing 16 Platform Subsystems...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-xl border border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-800/80 rounded-lg border border-slate-700 text-xs">
            ...
            <span className="text-slate-300 font-medium">Status: {data?.overallStatus || 'Operational'}</span>
          </div>
          <button
            onClick={handleManualProbe}
            disabled={refreshing}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-semibold shadow-xs disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Host Platform</span>
          <div className="text-sm font-bold text-slate-100">{hostStats.platform} ({hostStats.arch})</div>
          <p className="text-[10px] text-slate-500">Node.js Next.js Runtime</p>
        </div>
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Host Memory</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Host CPU Cores</span>
          <div className="text-sm font-bold text-slate-100">{hostStats.cpuCount} Cores</div>
      ...
      {/* Filter & Search */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
        <div className="flex items-center gap-2 w-full sm:w-72 bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3.5">
        {filteredSubsystems.map((sub) => {
          ...
          return (
            <div
              key={sub.id}
              className="bg-slate-900/90 border border-slate-800 hover:border-slate-700 rounded-xl p-4 flex flex-col justify-between space-y-3 transition-all duration-200 shadow-sm"
            >
              <div className="space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <h3 className="text-xs font-bold text-slate-100">{sub.name}</h3>
                  </div>
                  <span
              ...
              <div className="pt-2 border-t border-slate-800/80 grid grid-cols-2 gap-2 text-[10px]">
                <div>
                  ...
                  <span className="font-semibold text-slate-200">{sub.responseTimeMs} ms</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Availability</span>
                  <span className="font-semibold text-slate-200">{sub.availabilityPct}%</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Throughput</span>
                  <span className="font-semibold text-slate-200">{sub.requestsPerMin} req/min</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Error Rate</span>
                  <span className={`font-semibold ${sub.errorRatePct > 0 ? 'text-amber-400' : 'text-slate-200'}`}>
