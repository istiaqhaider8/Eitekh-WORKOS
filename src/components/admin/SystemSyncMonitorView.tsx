  ...
  if (loading && !data) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-sm font-semibold text-slate-400">Loading System Sync Telemetry...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-in fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold text-slate-100">System Synchronization Monitor</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3">
          ...
          <div className="text-xl font-bold text-slate-100 mt-1">{metrics.activeConnections || 0}</div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3">
          ...
          <div className="text-xl font-bold text-slate-100 mt-1">{metrics.totalEventsProcessed || 0}</div>
          <div className="text-[10px] text-slate-400 mt-0.5">Database Mutations</div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3">
          ...
          <div className="text-xl font-bold text-emerald-400 mt-1">{metrics.successRate || 100}%</div>
          <div className="text-[10px] text-slate-400 mt-0.5">Zero Event Dropped</div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3">
          ...
          <div className="text-xl font-bold text-slate-100 mt-1">{metrics.dbLatencyMs || 2}ms</div>
          <div className="text-[10px] text-emerald-400 mt-0.5">Single Source of Truth</div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3">
          ...
          <div className="text-xl font-bold text-purple-400 mt-1">{metrics.apiHealth || 'OPTIMAL'}</div>
          <div className="text-[10px] text-slate-400 mt-0.5">PBAC Protected</div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3">
          ...
          <div className="text-xl font-bold text-slate-100 mt-1">{metrics.retryQueueSize || 0}</div>
          <div className="text-[10px] text-slate-400 mt-0.5">No Backpressure</div>
        </div>
      </div>
          ...
          {(metrics.modules || []).map((mod: any) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-slate-800/60 text-[10px]">
                ...
                <span className="font-mono text-slate-400">{mod.latencyMs}ms</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeClients.length > 0 && (
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">Authenticated SSE Connections</span>
          </div>
                ...
                {activeClients.map((client: any) => (
                  <tr key={client.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="py-2.5 px-3">
                      <div className="font-bold text-slate-200">{client.userName || client.userEmail}</div>
                      <div className="text-[10px] text-slate-400 font-mono">{client.userEmail}</div>
                    </td>
                    <td className="py-2.5 px-3 font-mono text-blue-400">{client.projectId}</td>
                    <td className="py-2.5 px-3">
                      {client.isSuperAdmin ? (
                    ...
                    <td className="py-2.5 px-3 text-slate-400">{new Date(client.connectedAt).toLocaleTimeString()}</td>
                    <td className="py-2.5 px-3 font-mono text-slate-400">{client.ipAddress || '127.0.0.1'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-xs text-slate-100 placeholder:text-slate-500 focus:outline-hidden focus:border-blue-500 w-36 sm:w-44"
              />
            </div>
            ...
            <select
              ...
              className="px-2.5 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-xs text-slate-300"
            >
            ...
            <tbody className="divide-y divide-slate-800/60">
              {eventLogs.map((log: any) => (
                <tr key={log.eventId} className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-2.5 px-3 font-mono text-[11px] text-slate-300">{log.eventId}</td>
                  <td className="py-2.5 px-3">
                    <span
                  ...
                  <td className="py-2.5 px-3 font-mono text-slate-400">{log.projectId}</td>
                  <td className="py-2.5 px-3 text-slate-300">{log.actor?.email || 'System'}</td>
                  <td className="py-2.5 px-3">
                    <span
                  ...
                  <td className="py-2.5 px-3 font-mono text-slate-300">{log.recipientsCount} client(s)</td>
            ...
            <div className="p-4 overflow-y-auto space-y-3 flex-1">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <div className="font-bold text-blue-400">{selectedLog.eventType}</div>
                </div>
                <div>
                  <span className="text-slate-400">Project Scope:</span>
                  <div className="font-mono text-slate-200">{selectedLog.projectId}</div>
                </div>
              </div>

              <div>
                <span className="text-xs text-slate-400 block mb-1">Target Synchronized Modules:</span>
              ...
              <div>
                <span className="text-xs text-slate-400 block mb-1">Raw Payload JSON:</span>
                <pre className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-[11px] font-mono text-emerald-300 overflow-x-auto max-h-64">
